...
for swift in `find $1 ! -name "$destination_filename" ! -name "main.swift" -name "*+*.swift"`;
do
...
for swift in `find $1 ! -name "$destination_filename" ! -name "*+*.swift" ! -name "main.swift" -name "*.swift"`;
do
  filename=$(basename "$swift")
...
for swift in `find $1 -name "main.swift"`;
do
  filename=$(basename "$swift")
